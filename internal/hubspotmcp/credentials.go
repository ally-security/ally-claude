...
func SaveClientCredentials(clientID, clientSecret string, callbackPort int) error {
	...
	if clientID == "" || clientSecret == "" {
		return fmt.Errorf("hubspot client_id and client_secret are required")
	}
	if callbackPort == 0 {
...
func LoadClientCredentials() (ClientCredentials, error) {
	...
	if err != nil {
		return ClientCredentials{}, fmt.Errorf("no HubSpot OAuth client in Keychain — run: ally3p claude sync <policy.yaml>")
	}
	...
	if c.ClientID == "" || c.ClientSecret == "" {
		return ClientCredentials{}, fmt.Errorf("HubSpot OAuth client in Keychain is incomplete — re-run ally3p claude sync")
	}
	if c.CallbackPort == 0 {
